...
corelist="snes,nes,genesis,tgfx16cd,arcade,megacd"
timer=120
pathfs=/media/fat
...
{
	# Check if roms are zipped
	if [ -z "$(find $pathfs/Games/NES -maxdepth 1 -type f \( -iname "*.zip" \))" ] 
	then 
		#echo "Your rom archive seems to be unzipped" 
		NESrom="$(find $pathfs/Games/NES -type d \( -name *Eu* -o -name *BIOS* -o -name *Other* -o -name *FDS* \) -prune -false -o -name '*.nes' | shuf -n 1)"
		echo "Next up on the Nintendo Entertainment System:"
		echo -e "\e[1m $(echo $(basename "${NESrom}") | sed -e 's/\.[^.]*$//') \e[0m"
		echo "$(echo $(basename "${NESrom}") | sed -e 's/\.[^.]*$//') (NES)" > /tmp/Attract_Game.txt
	else 
		#echo "Need to use partun for unpacking random roms"
		if [ -f "${partunpath}" ]; then
			#echo "Partun installed. Launching now"
			NESrom=$("${partunpath}" "$(ls $pathfs/Games/NES/\@NE*.zip | shuf -n 1)" -i -r -f nes --rename /tmp/NEStmp.nes)
			echo "Next up on the Nintendo Entertainment System:"
			echo -e "\e[1m $(echo $(basename "${NESrom}") | sed -e 's/\.[^.]*$//') \e[0m"
			echo "$(echo $(basename "${NESrom}") | sed -e 's/\.[^.]*$//') (NES)" > /tmp/Attract_Game.txt
			NESrom=/tmp/NEStmp.nes
		else
			get_partun
			NESrom=$("${partunpath}" "$(ls $pathfs/Games/NES/\@NE*.zip | shuf -n 1)" -i -r -f nes --rename /tmp/NEStmp.nes)
			echo "Next up on the Nintendo Entertainment System:"
	...
	if [ -z "$NESrom" ]; then
		echo "Something went wrong. There is no valid file in NESrom variable."
		loop_core
	fi
  ...
  if [ -f "${mbcpath}" ] ; then
	...
	"${mbcpath}" load_rom NES "$NESrom" > /dev/null 2>&1
